...
// ========== AI工具函数 ==========
async function generateSiteDescription(url, env) {
  try {
    // 先检查缓存
    const cached = await getCachedDescription(url, env);
    if (cached) {
      console.log(`使用缓存的描述: ${url}`);
      return cached;
    }
    ...
    // 使用Cloudflare Workers AI生成描述
    const aiResponse = await env.AI.run('@cf/meta/llama-3-8b-instruct', {
      messages: [
        {
          role: "system",
          content: "你是一个专业的网站描述生成助手。请根据网站URL生成简洁、准确的中文描述，不超过50个字。描述应该突出网站的主要功能和特点。"
        },
        {
          role: "user", 
          content: `请为网站 ${url} (域名: ${domain}) 生成一个简洁的中文描述。`
        }
      ],
      max_tokens: 100
    });
    ...
    if (aiResponse && aiResponse.response) {
      ...
      // 缓存生成的描述
      await setCachedDescription(url, description, env);
      return description;
    }
    return null;
  } catch (error) {
...
async function getCachedDescription(url, env) {
  const cacheKey = `ai_desc_${url}`;
  return await env.NAV_CACHE.get(cacheKey);
}

async function setCachedDescription(url, description, env) {
  const cacheKey = `ai_desc_${url}`;
  // 缓存30天
  await env.NAV_CACHE.put(cacheKey, description, {
    expirationTtl: 30 * 24 * 3600
  });
}

async function checkAIUsageLimit(env, userId = 'anonymous') {
  const usageKey = `ai_usage_${userId}`;
  const today = new Date().toISOString().split('T')[0];
  const dailyUsageKey = `${usageKey}_${today}`;
  
  const currentUsage = await env.NAV_CACHE.get(dailyUsageKey) || '0';
  const dailyLimit = 50; // 每日50次
  
  if (parseInt(currentUsage) >= dailyLimit) {
    return {
      allowed: false,
      remaining: 0,
      total: dailyLimit,
  ...
  await env.NAV_CACHE.put(dailyUsageKey, newUsage.toString(), {
...
async function generateDescriptionForSite(request, env, ctx) {
  try {
    ...
    // 检查使用量限制
    const usageCheck = await checkAIUsageLimit(env);
    
    if (!usageCheck.allowed) {
      return new Response(JSON.stringify({ 
        success: false, 
        error: `今日AI生成次数已达上限 (${usageCheck.total}次)，请明天再试`,
        usage: usageCheck
      }), {
        status: 429,
        headers: { 'Content-Type': 'application/json' }
    ...
    const description = await generateSiteDescription(url, env);
    ...
    if (description) {
      ...
      // 如果生成失败，回滚使用计数
      const usageKey = `ai_usage_anonymous`;
      const today = new Date().toISOString().split('T')[0];
      const dailyUsageKey = `${usageKey}_${today}`;
      const currentUsage = await env.NAV_CACHE.get(dailyUsageKey) || '0';
      await env.NAV_CACHE.put(dailyUsageKey, (parseInt(currentUsage) - 1).toString(), {
        expirationTtl: 86400
      });
...
async function getAIUsage(request, env, ctx) {
  try {
    const usageKey = `ai_usage_anonymous`;
    const today = new Date().toISOString().split('T')[0];
    const dailyUsageKey = `${usageKey}_${today}`;
    ...
    return new Response(JSON.stringify({
      ...
      total: dailyLimit,
      resetTime: new Date(new Date().toDateString()).getTime() + 86400000 // 明天0点的时间戳
    }), {
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (error) {
    return new Response(JSON.stringify({ 
      error: '获取使用量失败: ' + error.message 
    }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  ...
  async handleRequest(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/ai/generate-description' && method === 'POST') {
        return await generateDescriptionForSite(request, env, ctx);
      }
      
      if (path === '/ai/usage' && method === 'GET') {
        return await getAIUsage(request, env, ctx);
      }
...
<body>
  <div class="container">
      ...
      <div class="import-export">
        ...
        <button id="batchGenerateDescBtn">批量生成描述</button>
      </div>

      <div id="aiUsageDisplay" class="ai-usage-display"></div>
      ...
      <div class="add-new">
        ...
        <div class="relative">
          ...
          <button type="button" id="generateAddDescBtn" class="ai-generate-btn" title="AI生成描述">
...
const batchGenerateDescBtn = document.getElementById('batchGenerateDescBtn');
const generateAddDescBtn = document.getElementById('generateAddDescBtn');
...
async function updateAIUsageDisplay() {
  try {
    ...
    if (data.used !== undefined) {
      const usageDiv = document.getElementById('aiUsageDisplay');
      if (usageDiv) {
        const percentage = (data.used / data.total) * 100;
        usageDiv.innerHTML = \`
          <div class="ai-usage-stats">
            <span class="font-semibold">AI描述生成统计</span>
            <span>\${data.used}/\${data.total} (剩余: \${data.remaining})</span>
          </div>
          <div class="ai-usage-progress">
            <div class="ai-usage-progress-bar" style="width: \${percentage}%"></div>
          </div>
          <div class="ai-usage-info">
            重置时间: \${new Date(data.resetTime).toLocaleString()}
          </div>
        \`;
      }
      
      // 更新按钮状态
      if (generateAddDescBtn) {
        generateAddDescBtn.disabled = data.remaining <= 0;
        generateAddDescBtn.title = data.remaining > 0 ? \`剩余\${data.remaining}次生成机会\` : '今日AI生成次数已达上限';
      }
      
      if (batchGenerateDescBtn) {
        batchGenerateDescBtn.disabled = data.remaining <= 0;
        batchGenerateDescBtn.title = data.remaining > 0 ? \`批量生成 (剩余\${data.remaining}次)\` : '今日AI生成次数已达上限';
      }
    }
  } catch (error) {
    console.error('获取AI使用量失败:', error);
  }
}

// AI生成描述
async function generateAIDescription(url, textareaId) {
  if (!url) {
    showMessage('请先输入网站URL', 'error');
    return;
  }
  ...
  try {
    ...
    if (data.success && data.description) {
      ...
      showMessage('AI描述生成成功', 'success');
      updateAIUsageDisplay();
    } else {
      showMessage(data.error || 'AI生成描述失败', 'error');
    }
  } catch (error) {
...
// 添加AI生成按钮事件
if (generateAddDescBtn) {
  generateAddDescBtn.addEventListener('click', () => {
    const url = addUrl.value.trim();
    generateAIDescription(url, 'addDesc');
  });
}

// 批量生成描述
if (batchGenerateDescBtn) {
  batchGenerateDescBtn.addEventListener('click', async () => {
    if (!confirm('确定要为所有没有描述的网站生成AI描述吗？这可能需要一些时间。')) {
      return;
    }
    ...
    try {
      ...
      if (usageData.remaining <= 0) {
        showMessage('今日AI生成次数已达上限，请明天再试', 'error');
        return;
      }
      
      const sitesWithoutDesc = sitesData.filter(site => !site.desc || site.desc.trim() === '');
      
      if (sitesWithoutDesc.length === 0) {
        showMessage('没有需要生成描述的网站', 'info');
        return;
      }
      
      // 限制批量生成的数量不超过剩余次数
      const maxBatchSize = Math.min(sitesWithoutDesc.length, usageData.remaining);
      const sitesToProcess = sitesWithoutDesc.slice(0, maxBatchSize);
      
      if (maxBatchSize < sitesWithoutDesc.length) {
        const confirmed = confirm(\`只能为前\${maxBatchSize}个网站生成描述（剩余\${usageData.remaining}次）。是否继续？\`);
        if (!confirmed) return;
      }
      ...
      batchGenerateDescBtn.disabled = true;
      batchGenerateDescBtn.textContent = '生成中...';
      ...
      for (const site of sitesToProcess) {
        try {
          ...
          if (data.success && data.description) {
            // 更新网站描述
            await fetch(\`/api/config/\${site.id}\`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json'
              },
            ...
            if (data.usage && data.usage.remaining <= 0) {
              showMessage('AI生成次数已达上限，批量生成已停止', 'warning');
              break;
            }
          }
        } catch (error) {
          console.error(\`生成描述失败 (\${site.url}):\`, error);
          failCount++;
        }
      ...
      batchGenerateDescBtn.disabled = false;
      batchGenerateDescBtn.textContent = '批量生成描述';
      
      showMessage(\`批量生成完成！成功: \${successCount}, 失败: \${failCount}\`, 'info');
      fetchCategoriesWithSites();
      updateAIUsageDisplay();
    } catch (error) {
      console.error('批量生成描述错误:', error);
  ...
  <div class="modal-content">
    ...
    <form id="editForm">
      ...
      <div class="relative">
        ...
        <button type="button" id="generateEditDescBtn" class="ai-generate-btn" title="AI生成描述">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
        </button>
...
// 编辑模态框中的AI生成按钮
const generateEditDescBtn = document.getElementById('generateEditDescBtn');
if (generateEditDescBtn) {
  generateEditDescBtn.addEventListener('click', () => {
    const url = document.getElementById('editUrl').value.trim();
    generateAIDescription(url, 'editDesc');
  });
}
...
// 每5分钟更新一次AI使用量
setInterval(updateAIUsageDisplay, 300000);`
    };
      ...
      <div class="p-6">
        ...
        <div class="mb-4 p-3 bg-gray-50 rounded-lg">
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600">AI描述生成</span>
            <span id="aiUsageDisplay" class="text-sm text-gray-600">加载中...</span>
          </div>
        </div>
        ...
        <form id="addSiteForm" class="space-y-4">
          ...
          <div class="relative">
            ...
            <div class="mt-1 flex">
              <textarea id="addSiteDesc" rows="2" class="flex-1 px-3 py-2 border border-slate-200 rounded-l-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400"></textarea>
              <button type="button" id="generateDescBtn" class="px-3 py-2 bg-accent-500 text-white rounded-r-md hover:bg-accent-600 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed" title="AI生成描述">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </button>
            </div>
            <div id="descLoading" class="hidden mt-1 text-sm text-accent-600">
              正在生成描述...
            </div>
          </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      async function updateAIUsageDisplay() {
        try {
          ...
          if (data.used !== undefined) {
            const usageElement = document.getElementById('aiUsageDisplay');
            if (usageElement) {
              usageElement.textContent = \`AI生成: \${data.used}/\${data.total} (剩余: \${data.remaining})\`;
              usageElement.className = data.remaining > 0 ? 'text-sm text-gray-600' : 'text-sm text-red-600';
            }
            ...
            if (generateBtn) {
              ...
              if (data.remaining <= 0) {
                generateBtn.title = '今日AI生成次数已达上限';
              } else {
                generateBtn.title = \`剩余\${data.remaining}次生成机会\`;
              }
            }
          }
        } catch (error) {
      ...
      const addSiteDesc = document.getElementById('addSiteDesc');
      ...
      // AI生成描述功能
      if (generateDescBtn && addSiteUrl && addSiteDesc) {
